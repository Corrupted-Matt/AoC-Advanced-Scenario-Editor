...
namespace AoC_Advanced_Scenario_Editor
{
    public partial class V5UI : KryptonForm
    {
        ...
        private void V5UI_Load(object sender, System.EventArgs e)
        {
        ...
        private void MapZoomChanged(object sender, MouseEventArgs e)
        {
            ...
            if (e.Delta > 0 && ZoomLevel < 16) ZoomLevel *= 2;
        ...
        private void UpdateCities(JsonNode Scenario)
        {
            ...
            cityX.Maximum = (int)origin["width"] - 1;
            cityY.Maximum = (int)origin["height"] - 1;
        ...
        #region Links
        private void ReadmeLink_LinkClicked(object sender, EventArgs e)
        {
            Process.Start(new ProcessStartInfo
            {
        ...
        private void DiscordLink_LinkClicked(object sender, EventArgs e)
        {
            Process.Start(new ProcessStartInfo
            {
        ...
        private void ConverterLink_LinkClicked(object sender, EventArgs e)
        {
            Process.Start(new ProcessStartInfo
            {
        ...
        private void FlagEditorLink_LinkClicked(object sender, EventArgs e)
        {
            Process.Start(new ProcessStartInfo
            {
        ...
        private void KryptonLink_LinkClicked(object sender, EventArgs e)
        {
            Process.Start(new ProcessStartInfo
            {
                FileName = "https://github.com/Krypton-Suite/Standard-Toolkit",
                UseShellExecute = true
            });
        }
        ...
        public void DrawGlobalMaps(JsonNode Scenario)
        {
            ...
            for (int y = h - 1; y >= 0; y--)
            {
                for (int x = 0; x < w; x++)
                {
                    ...
                    {
                        case 0:
                            maps[0].SetPixel(x, y, Color.Transparent);
                        ...
                        case 1:
                            ...
                            break;
                        case 2:
                            maps[0].SetPixel(x, y, Color.FromArgb(100, 0xcc, 0xcc, 0xcc));
                            break;
                        case 3:
        ...
        private Bitmap DrawZoomedMap(JsonNode Scenario, int CenterX, int CenterY)
        {
            ...
            int minX = Math.Max(0, CenterX - 480 / ZoomLevel), minY = Math.Max(0, h - 1 - CenterY - 240 / ZoomLevel);
            int maxX = Math.Min(w, CenterX + 480 / ZoomLevel), maxY = Math.Min(h, h - 1 - CenterY + 240 / ZoomLevel);
            ...
            if (ShowGrid.Checked && TabSelect.SelectedIndex == 3)
                using (Graphics g = Graphics.FromImage(grid))
                {
                    if (ZoomLevel >= 8)
                    {
                        for (int x = ZoomLevel / 2; x < grid.Width; x += ZoomLevel)
                        {
                            g.DrawLine(new(Color.FromArgb(50, Color.Black)), x, 0, x, grid.Height - 1);
                        }
                        for (int y = ZoomLevel / 2; y < grid.Height; y += ZoomLevel)
                        {
                            g.DrawLine(new(Color.FromArgb(50, Color.Black)), 0, y, grid.Width - 1, y);
                        }
                    }

                    for (int x = (4 - minX) % 5 * ZoomLevel + ZoomLevel / 2; x < grid.Width; x += 5 * ZoomLevel)
                    {
                        g.DrawLine(new(Color.FromArgb(150, Color.Black)), x, 0, x, grid.Height - 1);
                    }
                    for (int y = (h - 1 - minY) % 5 * ZoomLevel + ZoomLevel / 2; y < grid.Height; y += 5 * ZoomLevel)
                    {
            ...
            {
                g.InterpolationMode = System.Drawing.Drawing2D.InterpolationMode.NearestNeighbor;
                g.DrawImage(minimap, 0, 0, minimap.Width * ZoomLevel, minimap.Height * ZoomLevel);
        ...
        private Point GetMapClickPos(DataGridView Table, PictureBox Box, int ClickX, int ClickY)
        {
            ...
            {
                minX = Math.Max(0, CenterX - 480 / ZoomLevel); minY = Math.Max(0, h - 1 - CenterY - 240 / ZoomLevel);
                maxX = Math.Min(w, CenterX + 480 / ZoomLevel); maxY = Math.Min(h, h - 1 - CenterY + 240 / ZoomLevel);
